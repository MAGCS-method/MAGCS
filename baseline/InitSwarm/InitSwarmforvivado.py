...
def main():
    ...
    core_params = f"synth_design -top {params['top']} -flatten_hierarchy {params['flatten_hierarchy']} " \
                  f"-gated_clock_conversion {params['gated_clock_conversion']} -bufg {params['bufg']} " \
                  f"-directive {params['directive']} -fsm_extraction {params['fsm_extraction']} " \
                  f"-resource_sharing {params['resource_sharing']} -control_set_opt_threshold {params['control_set_opt_threshold']} " \
                  f"-shreg_min_size {params['shreg_min_size']} -max_bram {params['max_bram']} " \
                  f"-max_uram {params['max_uram']} -max_dsp {params['max_dsp']} " \
                  f"-max_bram_cascade_height {params['max_bram_cascade_height']} " \
                  f"-max_uram_cascade_height {params['max_uram_cascade_height']} -cascade_dsp {params['cascade_dsp']} " \
                  f"-incremental_mode {params['incremental_mode']}"

    optional_params = " ".join(random.sample(params["other"], len(params["other"])))
    synth_command = f"{core_params} {optional_params}"
